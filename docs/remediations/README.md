...
- `superseded`：原 in-progress runtime task/ref 已丢失，由带全局唯一 context 的新 REM 原子替代；必须记录 `superseded_by` 与 `supersession_reason: context-loss`，且不进入复审或整改队列。

`completed`、`partial`、`blocked` 或 `superseded` 的整改记录视为关闭且不可改写；继续整改创建新的 REM。恢复 in-progress REM 只允许原 `runtime_context_ref` 的可恢复 task；否则必须在同一精确事务中终止旧记录并建立新 open checkpoint，禁止接管。
...
每份 REM 必须由真实、独立的逐记录 runtime task/context 创建，使用全局唯一 `execution_context_id`，并在创建时立即加入本索引且只能出现一次。索引格式必须记录 `status` 与 `verification`：
...
`status=superseded` 固定使用 `verification=not-ready`，只表示 context-loss 替代，不得被 follow-up 或默认整改队列选择。
