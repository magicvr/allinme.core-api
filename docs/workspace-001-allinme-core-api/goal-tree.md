...
└── GOAL-002-mvp-demo-admin                 [active  50%]  MVP Admin · R1（渐进子目标）
    ...
    └── GOAL-006-wallet-api                 [active   0%]  钱包 API 与种子数据（契约门禁 open）
```
...
| GOAL-006-wallet-api | 钱包 API 与种子数据 | GOAL-002-mvp-demo-admin | **active** | 0% | I-001 实施契约 required/open；尚未编码 |
...
| R1 MVP Demo Admin | **进行中** | GOAL-002；GOAL-004/005 done；GOAL-006 active（钱包契约门禁） |
| R2 协议对齐与复用沉淀 | 未开始 | 待建 |
| R3 多项目消费就绪 | 未开始 | 待建 |
...
- **GOAL-006 I-001**：钱包首切片实施契约 required/open；阻断钱包 W1～W3 编码，下一步先冻结并自审契约。
