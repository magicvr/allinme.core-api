...
└── GOAL-002-mvp-demo-admin             [active  50%]  MVP Admin · R1（M2 完成；M3 订单首切片完成，钱包/通知待）
```
...
| GOAL-002-mvp-demo-admin | MVP · Demo 完整 Admin（协议驱动） | GOAL-001-allinme-core-api | **active** | 50% | JWT+菜单、M3 订单首切片完成；钱包/通知待 |
...
| R1 MVP Demo Admin | **进行中** | GOAL-002；M2 完成；M3 订单首切片完成，钱包/通知待 |
| R2 协议对齐与复用沉淀 | 未开始 | 待建 |
| R3 多项目消费就绪 | 未开始 | 待建 |
...
- **GOAL-002 M3**：订单首切片完成；钱包、通知 API 待实施（订单单项 DELETE/refund 后续补齐）。
