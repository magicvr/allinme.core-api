...
| 项目宪章、目标优先级、资产分层与防漂移规则 | [`00-overview.md`](./00-overview.md#2-项目宪章与防漂移规则) | README 只保留摘要，计划和路线只引用，不复制完整规则 |
...
5. 审计工作流是按风险选用的辅助入口，不是每个产品变更的默认前置。需要正式审计时保留独立记录和历史关系；计划归档仍须经用户确认。
