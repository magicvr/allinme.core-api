...
7. 订单 DTO 的 `canRequestRefund` 在阶段四改为动态值：主体为 `operator`/`admin`、paymentStatus 为 `PAID`/`PARTIALLY_REFUNDED` 且 `availableAmount > 0` 时为 `true`。订单列表必须批量加载当前页退款占用，禁止逐订单查询；订单 DTO 的 `canApproveRefund` 继续固定为 `false`，具体审批入口只由退款 DTO 的 `canApprove`/`canReject` 表达。
...
退款 aggregate 的校验顺序固定为：资源存在性 → version → `PENDING` 状态 → 申请/审批主体隔离 → 金额与订单支付状态。version 与状态同时不匹配时返回版本冲突。审批事务中任一步失败必须整体回滚，不允许出现退款 `COMPLETED` 但订单仍为 `PAID` 的中间状态。

`admin` 同时拥有申请和审批角色，但仍不得审批自己创建的退款。前端 capability 只用于展示；所有 endpoint 必须重新鉴权、校验 version 和状态。

订单退款 capability 属于阶段四现有订单查询契约的增量。`availableAmount = order.totalAmount - pendingAmount - completedAmount`；`canRequestRefund` 按 P0 冻结的角色、paymentStatus 和 availableAmount 计算。订单列表 repository 必须在当前页范围内批量取得退款占用并与订单 COUNT/page query 保持同一只读 SQLite snapshot；订单详情使用同一计算规则。退款创建成功后重新读取列表或详情必须立即反映新的 availableAmount/capability。订单没有可定位到具体退款的 ID，因此 `canApproveRefund` 保持 `false`，审批能力只出现在退款列表 DTO。
...
退款列表至少支持 `status`、`orderId`、`page`、`pageSize`，默认 `createdAt desc, id desc`；筛选、分页和重复 query 规则沿用订单列表。退款 DTO 至少包含退款 ID、订单 ID、金额、币种、原因、状态、version、申请人、审批人、时间和按当前主体计算的 `canApprove`/`canReject`。

幂等 header、字符集、长度和错误语义复用阶段三冻结值。作用域为 `principal user ID + POST + /api/v1/orders/{orderId}/refunds operation + key`；operation 必须使用规范化 route 模板而不是原始 path，normalized request digest 必须包含规范化 order ID，避免同一 key 和相同 body 在不同订单间错误重放。完成 transport、主体授权、key 和 normalized request 校验后，repository/service 必须先查幂等记录；命中同一请求时直接校验并返回首次 refund snapshot v1，不再检查当前 order version、paymentStatus、availableAmount 或当前退款状态。不同 request digest 返回 `409 IDEMPOTENCY_CONFLICT`。审批完成后再次重放原创建请求仍返回首次 snapshot，且不读取当前退款或订单补字段、不重复占用额度、不更新时间。
...
- 实现可退金额、申请占用、snapshot v1、approve/reject 事务、订单支付状态更新和现有订单 `canRequestRefund` 动态计算；
- 订单列表批量加载退款占用，列表/详情复用同一 capability 规则，`canApproveRefund` 保持 `false`；
- 覆盖损坏数据、超额、重复 key、不同 body、并发申请、并发审批、旧 version、self-approval 和 rollback；
...
3. 审批成功后退款、订单 paymentStatus、订单 version 和 dashboard 在同一提交后保持一致；订单列表/详情的 `canRequestRefund` 与实际 availableAmount 一致且无逐订单查询，`canApproveRefund` 保持 `false`；
