...
- 上传先产生当前用户拥有、24 小时有效的未绑定附件，Schema-UI UploadAction 可直接取得稳定附件 ID；
...
阶段完成不代表附件页面已实现。阶段六才创建订单附件页面和完整 Schema-UI 页面配置；本阶段只冻结并交付 UploadAction、订单绑定和下载所需的 HTTP 契约。
...
1. schema 目标为 `PRAGMA user_version = 7`，固定产物路径为 `artifacts/phase5/v7/<artifact-kind>/<revision>/`，原始证据位于其 `evidence/` 子目录。v6 gate、v7 schema-only、5A feature 三类产物分别绑定不可变源码 revision、构建命令、文件名与 SHA-256，并在 binary manifest 中写入机器可验证的 `artifactKind`、支持 schema、允许数据阶段和 capability 标识；“同一候选 revision”只约束最终 5A feature binary，不得把三个历史产物伪装成同一 revision。
...
36. P0 交付拆成五个独立工作包并分别记录 owner、输入、出口条件、最大 timebox 和 revision/Evidence：事实源同步；v7 migration/trigger/journal 草案；stable applock 与入口迁移；Windows/Linux files 原型；crash/release harness 与监督器/调度 profile。工作包可并行审查，但全部出口条件满足前不得进入 M1A，也不得把 ROM 当作阶段日期承诺。
...
38. 所有构造期入口共用一个 startup coordinator/typed gate API，统一覆盖 `NewAPI`、`NewAuthenticatedAPI`、`Handler()`、embedded entry、migrate、seed、reset、cleanup 和 bootstrap-admin 的允许阶段。schema-only admin 的 `migrate` 子命令是唯一例外：它在打开业务 service 前允许读取 v6/v7 并仅执行 v6→v7 或幂等 v7 校验；其他 admin 子命令和 API 只允许 v7。部署顺序、每步允许的 schema/data、失败恢复点、退出码和 binary SHA-256 必须形成可执行链路。
39. P0 必须交付机器可解析的 Evidence/manifest 最小工具链，放在 `internal/testkit/phase5` 或等价受审计目录：固定 JSON schema、全局唯一 run ID、Owner、Date/Timezone、Revision、Environment、Command、Artifact/Log Path、Result、Failure/Not-Run Reason 和 artifact kind；validator 必须核对 revision、文件 SHA-256、退出码、路径存在性及 manifest 与 run 记录的一致性。P0-14、P0-19、P0-22 各自产生至少一份真实样例和 validator 输出；仅有 Markdown 勾选或第三方报告不能作为完成 Evidence。
...
迁移只增加 schema，不扫描或改写现有订单、退款和幂等 snapshot。当前 v6 实际没有附件表或附件数据，因此无需迁移旧 Attachment；migration 必须覆盖空库、标准 v6、存在空的非预期旧附件表和存在旧附件数据四类 fixture，后两类直接拒绝并回滚，禁止猜测迁移。v7 migration 前先交付一个仍运行 v6 schema、但在打开业务 service/构造 handler/监听前强制 schema gate 的过渡 binary，证明它看到 `user_version > 6` 会直接退出；随后构建不可变 schema-only `api/admin`，停服并备份后由该 admin 的专用 `migrate` 子命令执行 v6→v7，再由同一 revision/hash 的 api/admin 验证空数据 gate。schema-only artifact 仅用于迁移与首次附件写入前的前滚准备；该 revision 的构造期 gate 固定执行：
...
|---|---|---|---|---|
| `v6-gate` | `phase5-v6-gate-api-<goos>-<goarch><ext>`；`go build -trimpath -o <file> ./cmd/api` | 仅 v6；不得存在 v7 表 | `user_version != 6`，退出 78 | 第 1 步部署并验证；迁移前成套备份；失败时仍运行该不可变 revision |
| `v7-schema-only` | `phase5-v7-schema-api-<goos>-<goarch><ext>` 与 `phase5-v7-schema-admin-<goos>-<goarch><ext>`；分别执行 `go build -trimpath -o <api-file> ./cmd/api`、`go build -trimpath -o <admin-file> ./cmd/admin` | API/其他 admin 仅 v7 且四张附件表为空、无 snapshot v2；admin `migrate` 在业务装配前允许 v6→v7 或幂等校验 v7 | API/其他 admin：schema 非 v7、任一附件行或 snapshot v2 时退出 78；`migrate` 对非 v6/v7 退出 78，对兼容性校验或执行失败退出 1，成功退出 0 | 第 2 步停服后由该 admin 执行迁移，再以同 SHA-256 的 api/admin 验证；首次附件写入前可恢复迁移前成套备份 |
...
| 里程碑 | 主要交付 | 预计工作量 |
|---|---|---:|
| P0 契约与原型 | 先同步事实源，产出 mapping/migration/幂等事务草案，完成 `os.Root`/deadline/文件身份、打开句柄 rename、运行契约和完整锁矩阵原型 | 6–10 天条件 ROM |
| M1A 5A 存储与恢复 | migration v7、`order_attachments`、`internal/files`、STAGING/单项 delete/expiry journal、构造期 schema gate/启动恢复、运行期 watchdog | 6–9 天 ROM |
| M2 HTTP 文件闭环 | upload/download/delete、JWT/CORS、deadline、下载 semaphore/header 矩阵 | 3–5 天 |
| M3A create 与幂等 v2 | create 绑定、`CreateResult`、DTO、snapshot v1/v2、并发 winner 重放 | 4–6 天 |
| 5A-I 实现门禁 | 上传、未绑定删除、create 绑定、普通 PATCH 无损、鉴权下载、v2 幂等、基础启动恢复与 watchdog 的本机 smoke/crash | 3–5 天 |
| 5A-D 部署门禁 | 三产物矩阵、cleanup 调度、进程监督、成套回退、ENOSPC、Windows/Linux 证据与发布说明 | 2–4 天 |
| M1B 组 journal 与恢复 | REMOVING、group journal、edit-remove/ORDER_DELETE、orphan claim 与完整组恢复 | 4–6 天 ROM |
| M3B edit 与删除原语 | edit 组移除、窗口语义、并发不变量、订单删除 cleanup 原语 | 5–8 天 |
| M4/5B/R 运维与最终发布 | 完整 cleanup/orphan、seed/reset、readiness 扩展、完整 crash smoke、文档和 CI | 4–7 天 |

默认按 5A/5B 两批交付，不把拆分作为压缩工期时的可选项。当前各项数字均为条件 ROM，不是排期或阶段日期承诺；单人完整范围暂按 37–60 个开发日管理。P0 的 6–10 天仅在无需自定义 syscall 且 Windows/Linux 原型一次通过时成立；若 Windows 需要专用打开/rename 层，P0 和 M1A 都必须单独增加缓冲。P0 五个工作包结束后，必须按实测平台适配层、crash harness、锁实现、startup coordinator、支持规模和 5A 部署 profile，重新估算 M1A、M2、M3A、5A-D、M1B/M3B/M4 与 5B，并记录 owner、review 带宽、环境可用性、关键路径和资源不足时的降级顺序；重估完成前不得承诺交付日期。降级顺序固定为先取消持续部署批准，再推迟 5B edit/order-delete/orphan，最后才讨论缩小支持规模，恢复、安全、race 和 Evidence 门禁不得降级。并行时由单一 owner 持有 file/journal/recovery 协议，另一 owner 持有 HTTP/order integration；事务协议变更仍须串行 review。5A 只实现 STAGING、单项 delete/expiry、create bind 和对应恢复；5A binary 对 group/REMOVING/ORDER_DELETE/ORPHAN_FINAL 等不支持状态在构造 handler 前 fail-closed。5B 再交付 group journal、edit 组移除、完整 cleanup/orphan、订单删除 cleanup 原语和完整 crash matrix。
...
- [配套 checklist](./0005-2026-07-13-checklist.md) 全部完成并记录实际 Evidence；
- `go test ./... -count=1`、`go vet ./...`、独立 `go test -race ./... -count=1` 通过；
- `docs/audit/validate.ps1`、`docs/audit/validate.tests.ps1` 和 `git diff --check` 通过；
- 新数据目录完成 migrate + seed + API 启动 + 登录 + 上传 + 创建绑定 + 下载 + 编辑替换/移除 + 删除未绑定 + cleanup + 重启 smoke；
- 上传失败、DB 提交失败、rename/remove/open/hash 失败以及进程在每个 rename/commit 边界退出后，均无可访问孤儿、BOUND 缺失 final 或路径泄露；
- 5A-I 与 5A-D gate、schema-only 限制、回退恢复、远端 Linux CI、最终 revision 和剩余风险有记录；
...
本计划对根目录两份 2026-07-14 临时审计意见逐条复核，结论为“需要修改，完成 P0 Evidence 后方可进入 M1A”。本轮采纳并落地：P0-1 独立事实源 revision；schema-only admin 的可执行 v6→v7 迁移路径；journal 历史 order/attachment 标识不级联、不 RESTRICT 的墓碑语义；app→`cmd/api` shutdown reason/退出码传播；统一 startup coordinator；机器可验证 artifact capability 与负向部署 smoke；可版本化 Windows/Linux/ENOSPC/crash/supervisor harness；固定 Evidence 模板与 run ID；P0 五工作包、owner/timebox/出口条件和重估门禁；部署支持 profile；启动支持规模、冷/热缓存证据与 30 秒硬上限；新增依赖基线；机器可解析 Evidence validator 及 P0-14/P0-19/P0-22 真实样例门禁；协议变更清单与相关 gate 重触发规则；`internal/demo-only` 产物/README/部署标识。既有独立 `order_attachments`、durable journal、v1/v2 snapshot、原始 multipart header、下载有界缓冲、stable applock 和 fail-closed 恢复主线继续保留。

未直接采信审计报告中声称已经通过的 test、race、validator 或 diff 基线；这些结果不作为本计划 Evidence，仍由 P0 在计划入库后的准确 revision 实跑。没有采纳“仅凭文件名或人工 manifest 区分产物”，也没有把 Windows 与 Linux 同时宣称为支持部署环境；只有完成监督、调度、恢复和 ENOSPC 证据的 profile 才能进入 5A-D 批准。对 5A 磁盘风险继续采用“前移最小 cleanup + 无部署证据时禁止持续部署”，不在本阶段引入完整配额系统。5A-I 通过后可继续 5B 开发，但 5A-D 未通过前不得持续部署；P0-1、migration 执行链、journal 外键语义、startup/exit 协议、启动规模、artifact validator、完整锁矩阵和平台 harness 完成前保持 No-Go。
