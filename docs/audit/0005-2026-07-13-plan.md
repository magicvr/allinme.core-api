...
1. schema 目标为 `PRAGMA user_version = 7`，本地/CI 临时产物路径为 `artifacts/phase5/v7/<artifact-kind>/<revision>/`，原始日志位于其 `evidence/` 子目录。`/artifacts/` 被 Git 忽略，只能作为生成缓存，不能成为唯一 Evidence。v6 gate/bootstrap、v7 schema-only、5A feature 三类产物分别绑定不可变源码 revision、构建命令、文件名与各自 SHA-256，并在 binary manifest 中写入机器可验证的 `artifactKind`、支持 schema、允许数据阶段和 capability 标识；“同一候选 revision”只约束最终 5A feature binary，不得把三个历史产物伪装成同一 revision。
...
5. 原始文件名安全校验必须直接读取每个 part 的原始 `Content-Disposition` header，使用严格解析器且不得以 `multipart.Part.FileName()` 的平台相关 basename 结果作为安全输入。只允许一个 `filename` 或一个 `filename*` 参数，二者并存、重复参数、解析失败、非 `UTF-8''` 的扩展编码或非法 percent-encoding 均返回 `400`；解码后先做 NFC 规范化，再要求有效 UTF-8、无 NUL、无 `/` 或 `\\`、无任何 Unicode `Cc`、无双向 override/isolate 字符 U+202A..U+202E/U+2066..U+2069、不是 `.`/`..`，长度为 1..255 UTF-8 bytes。`filename*` 使用 RFC 5987 UTF-8 percent-encoding 返回规范化名称；ASCII fallback 只保留 `[A-Za-z0-9._-]`，每段其他字符折叠为单个 `_`，结果为空时使用 `attachment`，并截断到 120 ASCII bytes。任何响应 header 都不得直接拼接原值。
...
9. order create 新请求使用 digest/snapshot v2。digest v2 的 canonical JSON 字段顺序固定为 `operation/customerName/currency/items/attachmentIds`，缺失与空数组统一编码为 `[]`；历史 snapshot v1 和其 digest 永不改写。幂等查询先按主体/method/route/key 取得记录再按 `snapshotVersion` 分派：命中 v1 时只有缺失或空 `attachmentIds` 可按旧 digest 重放，任何非空数组一律返回 `409 IDEMPOTENCY_CONFLICT`；v2 使用新 digest。未知版本返回 internal，禁止放宽 v1 decoder 兼容 v2。
...
15. 文件存储只接受服务端生成的固定 basename，并以启动时打开且全生命周期持有的 Go 1.26 `os.Root` 访问 `temp/final/trash`；禁止把客户端文本拼接为路径。附件根目录及其父目录必须由服务账号独占写入，能够以同一 OS 身份或更高权限并发改写该目录的主体不在应用层 TOCTOU 防护承诺内。打开后必须以句柄确认普通文件并记录轻量身份：Unix 使用 `st_dev/st_ino`，Windows 使用 volume serial + file ID；hash 后到绑定事务若重新打开，必须比较身份、size、mtime 和冻结摘要。rename/restore/purge 必须通过 P0 平台原型证明 `os.Root` containment、同卷、目标不存在、symlink/reparse 拒绝和 rename 前后身份关联；若标准库不足，平台适配层使用等价 syscall。静态 `Clean`/前缀判断或先 `Lstat` 后按裸路径操作不能单独作为安全证据。
...
17. 下载使用同一个安全打开的普通文件句柄，在提交任何 `200/304` 响应头前把最多 10 MiB 内容读入有界缓冲并完成 size、SHA-256 和内容类型复核。下载在“读到 `BOUND` 映射、完成订单授权并打开身份匹配的 final 句柄”处线性化；在线性化点后即使 edit 进入 `REMOVING`，该在途下载仍允许完成，只有此前观察到 `REMOVING` 的请求返回 `503`。P0 必须分别在 Windows/Linux 用真实句柄冻结“读句柄保持打开 → 并发 `final -> trash` rename → 原句柄继续读”的平台语义和 Windows `CreateFile` share mode；若 Windows 不能在保持读句柄时 rename，edit-remove 必须稳定返回 unavailable/重试并保持 PREPARED，不得宣称跨平台并行成功。进程级下载校验/缓冲 semaphore 固定为 8，即最多 80 MiB 文件内容缓冲，不含 Go/runtime 与 HTTP 额外开销；未取得槽位不排队并返回 `503 SERVICE_UNAVAILABLE` 与 `Retry-After: 1`，请求取消、读取失败和 API 关闭必须释放槽位。永久元数据/内容损坏返回 `500 INTERNAL_ERROR`，临时 open/read/锁/资源不足返回 `503`；响应提交后的客户端取消或 socket 写失败只记录安全传输分类并中止。响应和日志不得包含本地路径。
...
26. P0-1 必须先同步路线图、领域模型和目标 API，完成后才能编写 schema/API：阶段五只交付订单删除可复用清理原语与直接测试，不新增订单删除 HTTP；目标 API 使用显式允许角色；领域模型承接附件状态、上限、owner/expiry/绑定与下载业务规则。transport 细节仍由本计划持有，实施完成后迁入当前 API。
...
29. 运行期 unresolved 采用仓库自洽的应用驱动契约：生产 watchdog 每 5 秒检查一次，clock/ticker 可注入；任一 operation 年龄严格超过 5 分钟时，在最多一个 tick 内把 `/readyz` 置为 not-ready，并只调用一次注入的 shutdown callback。callback 停止接收新请求、给予在途请求 15 秒宽限期，随后让 `cmd/api` 以固定退出码 `75` 退出；重复检测不得重复关闭资源或重复回调，无 probe 流量也必须触发。该 watchdog 只负责安全停服，不执行恢复或 cleanup。`cleanup-attachments` 由外部调度执行：仓库必须交付并实测至少一个受支持的单机调度示例，固定每 15 分钟一次、单次超时 10 分钟、禁止重叠、非零退出触发可观察告警；没有调度与进程监督证据时不得批准持续部署。
30. 启动同步扫描限定为结构门禁：恢复全部 journal 后，按 500 行批次检查 schema、组/明细完整性、状态/映射计数、服务端 basename、final 存在性、普通文件身份和记录 size；不得在启动路径读取所有稳定文件计算 SHA-256 或重新探测 content type。扫描使用注入 clock，生产总预算固定 30 秒硬上限，超时 fail-closed 并输出不含路径的诊断码；完整 hash/type 校验保留在 bind、download、cleanup 和只读 `admin verify-attachments --full` 中。支持规模、冷/热缓存基线和超预算运维步骤按第 35 项冻结。
...
34. 运行期退出使用一次性 shutdown-reason arbiter/typed `RunResult` 从 app 传播到 `cmd/api`，优先级固定为启动前 schema gate `78` > 监听/运行失败 `1` > watchdog restart `75` > 外部 SIGTERM/普通 Close `0`；重复原因幂等，较低优先级不能覆盖已选定原因，真实子进程必须覆盖竞争与重复触发。
...
36. P0 交付拆成八个独立工作包，并按下表把 P0-1 至 P0-24 恰好映射到一个主 owner。owner 是可追责角色，开始执行时必须在 Evidence 中补充具体人员；reviewer/批准人不能与主 owner 为同一人。工作包可并行审查，但全部出口条件满足前不得进入 M1A，也不得把 ROM 当作阶段日期承诺。
...
38. 所有构造期入口共用一个 startup coordinator/typed gate API，统一覆盖 `NewAPI`、`NewAuthenticatedAPI`、`Handler()`、embedded entry、migrate、seed、reset、cleanup 和 bootstrap-admin 的允许阶段。契约冻结为：构造期只返回 `StartupError{Stage, Kind, ExitCode, Err}`；运行结束只返回 `RunResult{Reason, ExitCode, Err}`，其中 `ExitReason` 至少区分 schema gate、监听/运行失败、watchdog restart 和外部停止。统一生命周期为“取得 API/maintenance lock → 打开 DB/root → migration/schema gate → recovery → integrity scan → service/handler 构造 → 释放 maintenance → serve”；任一步失败都按逆序关闭 handler/service、root、DB，再释放 maintenance/API lock。`Handler()` 只能在 coordinator 达到 ready 状态后取得，测试只能注入依赖和 clock，不能跳过阶段。schema-only admin 的 `migrate` 子命令是唯一例外：它在打开业务 service 前允许读取 v6/v7 并仅执行 v6→v7 或幂等 v7 校验；v6 bootstrap admin 只允许 v0..v6 并停在 v6；其他 admin 子命令和 API 只允许其 manifest 声明的 schema/data。部署顺序、每步允许的 schema/data、失败恢复点、退出码和各 binary SHA-256 必须形成可执行链路。
39. P0 必须交付机器可解析的 Evidence/manifest 最小工具链，放在 `internal/testkit/phase5` 或等价受审计目录：固定 JSON schema、全局唯一 run ID、Owner、Date/Timezone、Revision、Environment、Command、Artifact/Log Path、Result、Failure/Not-Run Reason、artifact kind、process exit code、下载 URL、保留截止时间和内容 SHA-256；validator 必须核对 revision、文件 SHA-256、退出码、路径/URL 可取得性及 manifest 与 run 记录的一致性。`internal/testkit/phase5/testdata/evidence/` 保存可在 clean checkout 复核的正反样例；真实 run 的小型 manifest/index 提交到 `docs/audit/evidence/phase5/<run-id>/`，大型 binary/log/DB bundle 保存到带不可变 run ID 的 CI artifact 或外部只读存储。普通 P0/M1 Evidence 至少保留 180 天，最终部署批准 bundle 至少保留到对应支持版本生命周期结束；`artifacts/` 本机副本不得作为唯一来源。P0-14、P0-19、P0-22 各自产生至少一份真实样例和 validator 输出；仅有 Markdown 勾选或第三方报告不能作为完成 Evidence。
40. 对 P0-1、P0-2、P0-13、P0-17、P0-19、P0-20 建立协议变更清单。任何事务顺序、状态、DTO、错误码、锁/启动语义或平台能力变化，必须在同一独立 revision 同步更新 plan、checklist、事实源和 Evidence 模板，记录受影响里程碑并重新触发相关 gate；禁止通过实现分支或发布说明局部放宽门禁。

P0 tracked work-package 表如下。`输入 revision` 在工作包开始时必须替换为完整 commit；最大 timebox 是单包上限，不是日期承诺，超时必须停止扩范围并触发重估。

| 工作包 | 主 P0 条目 | owner / reviewer | 输入 revision 与阻塞项 | 最大 timebox | 出口 Evidence 与变更批准 |
|---|---|---|---|---:|---|
| WP-Facts | P0-1 | 阶段五协议 owner / domain+API reviewer | 本计划修订 revision；无代码前置 | 1 天 | 四份事实源 diff、行号、独立 revision；协议 owner 批准 |
| WP-Schema-Recovery | P0-2、P0-4、P0-9、P0-10 | storage/recovery owner / runtime reviewer | WP-Facts；迁移与状态机草案 | 3 天 | migration/trigger、真值表、crash 边界与双进程 fixture；协议 owner 批准 |
| WP-HTTP-Order | P0-3、P0-5、P0-6、P0-7、P0-8、P0-11、P0-12 | HTTP/order owner / security reviewer | WP-Facts；现有 v1 fixture | 3 天 | multipart/DTO/幂等/header/error 固定 fixture；协议 owner 批准 |
| WP-Lock | P0-13 | runtime lock owner / storage reviewer | 现有 applock revision | 2 天 | Windows/Linux 三进程锁矩阵与入口迁移清单；runtime owner 批准 |
| WP-Baseline-Evidence | P0-14、P0-23、P0-24 | Evidence/tooling owner / release reviewer | 本计划修订 revision；留存 profile | 2 天 | schema、validator、clean-checkout 样例、真实 run index 与协议变更清单；release owner 批准 |
| WP-Files | P0-15 | file adapter owner / security reviewer | WP-Lock 接口；Go 1.26 | 3 天 | os.Root、身份、rename/share mode、reparse/symlink 原型；security reviewer 批准 |
| WP-Runtime | P0-17、P0-18、P0-20 | startup/runtime owner / app reviewer | WP-Lock；startup 类型契约 | 3 天 | coordinator 生命周期、cmd/api 退出码进程测试、规模 fixture/30 秒结果；runtime owner 批准 |
| WP-Release | P0-16、P0-19、P0-21、P0-22 | release owner / Evidence reviewer | 其他原型输出；可用部署环境 | 2 天 | 入口/版本矩阵、三产物 manifest、支持 profile、逐包重估与关键路径；阶段五协议 owner 批准 |
...
- `created_by_user_id`、`order_attachments.order_id` 与 `order_attachments.attachment_id` 外键均使用 `ON DELETE RESTRICT`，未来订单删除必须先调用附件清理原语；`attachment_file_op_groups.order_id`、`attachment_file_ops.original_order_id` 和历史 `attachment_file_ops.attachment_id` 不建立会级联或 RESTRICT 的外键，它们是恢复快照/墓碑标识，必须保留到 purge 完成；`group_id` 仅在组生命周期内受约束。BUILDING→PREPARED trigger、最终事务 token 校验和启动扫描负责补足一致性，并固定覆盖“订单已删除但 COMMITTED tombstone 仍存在”的 fixture；
...
|---|---|---|---|---|
| `v6-gate` | `phase5-v6-gate-api-<goos>-<goarch><ext>` 与 `phase5-v6-bootstrap-admin-<goos>-<goarch><ext>`；分别构建 `./cmd/api`、`./cmd/admin` | API 仅 v6 且 `sqlite_schema` 不存在 manifest 列出的任何阶段五 table/index/trigger，`idempotency_keys` 不存在 `snapshot_version >= 2`；bootstrap admin 只允许 v0..v6 并停在 v6 | API：版本不为 v6、存在任一阶段五对象或 snapshot v2 时退出 78；bootstrap admin 对 >v6 或对象冲突退出 78 | fresh 第 1 步由该 admin 执行 v0→v6；legacy 第 1 步部署 API gate；v6 迁移前成套备份；失败时恢复 fresh 目录或继续运行该不可变 v6 revision |
| `v7-schema-only` | `phase5-v7-schema-api-<goos>-<goarch><ext>` 与 `phase5-v7-schema-admin-<goos>-<goarch><ext>`；分别执行 `go build -trimpath -o <api-file> ./cmd/api`、`go build -trimpath -o <admin-file> ./cmd/admin` | API/其他 admin 仅 v7 且四张附件表为空、无 snapshot v2；admin `migrate` 在业务装配前允许 v6→v7 或幂等校验 v7 | API/其他 admin：schema 非 v7、任一附件行或 snapshot v2 时退出 78；`migrate` 对非 v6/v7 退出 78，对兼容性校验或执行失败退出 1，成功退出 0 | 第 2 步停服后由同一个 admin SHA 执行并复核迁移，再用同源码 revision 的 API 自身 SHA 验证；首次附件写入前可恢复迁移前成套备份 |
| `v7-5a-feature` | `phase5-v7-5a-api-<goos>-<goarch><ext>` 与 `phase5-v7-5a-admin-<goos>-<goarch><ext>`；分别执行 `go build -trimpath -o <api-file> ./cmd/api`、`go build -trimpath -o <admin-file> ./cmd/admin` | v7；允许 5A 状态，5B group/kind fail-closed；产物、CI 输出和部署说明必须标注 `internal/demo-only` | schema 非 v7、无法恢复或出现不支持 group/kind | 第 3 步启用；功能写入后的回退只允许恢复同一时点成套备份，不得换回 schema-only binary |

每行原始 Evidence 先生成到 `artifacts/phase5/v7/<artifact-kind>/<revision>/evidence/`，随后按第 39 项写入受版本控制的 run index 并上传到已冻结的保留存储；本机 ignored 路径本身不可用于批准。Evidence 记录部署命令、数据库路径、备份标识、允许数据扫描结果、退出码、下载 URL、保留截止时间和回退演练。每个 binary 通过编译注入或等价只读接口输出机器可验证的 capability manifest；部署前 validator 必须核对 manifest、文件 SHA-256、schema/data scan，并以“错误 binary + 错误 schema/数据”负向 smoke 证明拒绝。`v7-5a-feature` 及其 CI/release artifact 的 manifest、README 和部署文档必须同时标注 `internal/demo-only`，不得仅依赖文件名或人工说明；文件名只作为辅助标识。历史过渡产物以各自 revision/hash 关联；最终 5A test/race/smoke 的“同一候选 revision”不追溯约束前两行。
...
订单删除 cleanup 原语复用 `ORDER_DELETE` 分组 operation，对同一订单的全部 BOUND 附件一次取得 ownership；阶段五通过 repository/service 直接测试它，不注册订单删除路由。准备阶段只允许创建 PREPARED group/op、把全部附件 `BOUND -> REMOVING` 并隔离文件，不得删除订单或标记 COMMITTED。store 必须暴露单一最终写事务入口，在同一 SQLite commit 中重新验证订单与全部 token、删除 `order_attachments` 和订单聚合行、把附件 `REMOVING -> DELETING`，并把 group/op 标为 `COMMITTED`；只有该事务提交后才能报告订单删除成功。最终事务失败或进程退出时订单仍存在，整组按 PREPARED restore；直接测试必须断言成功后订单确实不存在，失败后订单和映射仍完整。任何恢复分支都不得让稳定 `BOUND` 元数据长期指向缺失 final，也不得让 `DELETING/REMOVING` 文件可下载。
...
下载按 attachment ID 读取元数据，要求 `BOUND` 且关联订单存在、可查看；`REMOVING` 在旧订单摘要中仍可见但下载固定返回 `503`。取得 8 槽 semaphore 后通过持久 `os.Root` 和服务端派生 basename 安全打开一次，把内容读入 10 MiB 有界缓冲并验证普通文件身份、size、SHA-256 和实际类型。格式非法和不存在 ID 均返回 `404`。只有完整性验证通过后才处理条件请求：`If-None-Match` 命中返回 `304`；否则忽略 `Range`、返回带 `Accept-Ranges: none` 的完整 `200` 与完整 bytes。逐状态 header 服从第 3 节第 16 项冻结矩阵。永久损坏为 JSON `500`，临时 I/O/资源不足为 JSON `503`；响应提交后的客户端取消或写失败只中止传输并写安全日志。

### 5.3 删除

删除短路顺序：route/method → CORS → authentication → role → attachment ID → owner/状态 → CAS 获取 operation ownership/写 PREPARED → 文件隔离 → COMMITTED。格式非法和不可见统一为 `404`；本人已绑定返回 `409 STATE_CONFLICT`；恢复中状态返回稳定 unavailable；重复删除在记录已不存在时返回 `404`，不伪装成幂等成功。只有 COMMITTED 后返回 `204`，后续 purge 失败由 tombstone 重试。
...
digest v2 canonical JSON 精确形状为 `{"operation":"POST /api/v1/orders","customerName":"...","currency":"CNY","items":[...],"attachmentIds":[]}`，摘要为该 UTF-8 JSON 的 SHA-256；`attachmentIds` 保留输入顺序，缺失和空数组都编码为 `[]`。同 key/v2 digest 重放返回首次 snapshot；相同 key 但 attachmentIds 顺序或其他事实不同返回 `409 IDEMPOTENCY_CONFLICT`。历史 v1 记录继续按旧算法重放且不读取当前附件，但同 key 请求含任何非空 `attachmentIds` 必须在 v1 digest 比较前返回冲突。
...
- 超过 1 小时且可由 attachment ID 识别的 temp；
- 超过 24 小时、名称/扩展名合法的 orphan final 候选。扫描只负责产生候选；cleanup 必须在 SQLite 写事务中按 attachment ID 重新检查 `attachments` 和活动 operation 均不存在，再插入唯一 `ORPHAN_FINAL` PREPARED claim。上传的 STAGING 事务反向拒绝已有 claim；claim 提交后、文件隔离前再次验证 token/phase，随后隔离并在后续执行 purge。无法取得 claim 或无法证明无引用时绝不删除。
...
development seed 生成小型、确定性的合法 PDF/PNG/JPEG fixture，至少覆盖一个已绑定附件和一个未绑定附件。首次 seed 必须复用上传的 `temp -> STAGING -> final -> UPLOADED` durable 协议，再通过正常绑定事务产生 `order_attachments/BOUND`，不得直接写 final + metadata；真实子进程测试覆盖 seed 的每个 commit/rename 边界。seed 重放只读验证 DB 元数据、映射、文件大小和 SHA-256；发现文件缺失或内容漂移时失败，不静默重建掩盖损坏。seed/reset 均要求 API 停止并取得 `API exclusive -> maintenance exclusive`；reset 删除受控附件目录前还要通过现有 reparse point/path containment 防线扩展验证。
...
| 里程碑 | 主要交付 | 开发 ROM | 环境等待 | review/修复缓冲 |
|---|---|---:|---:|---:|
| P0 契约与原型 | 八工作包、fresh/v6/v7 矩阵、`os.Root`/deadline/文件身份、运行契约、完整锁矩阵和 Evidence 工具链 | 6–10 天 | 1–3 天 | 1–2 天 |
| M1A 5A 存储与恢复 | migration v7、`order_attachments`、`internal/files`、STAGING/单项 delete/expiry journal、构造期 schema gate/启动恢复、运行期 watchdog | 6–9 天 | 1–2 天 | 1–2 天 |
| M2 HTTP 文件闭环 | upload/download/delete、JWT/CORS、deadline、下载 semaphore/header 矩阵 | 3–5 天 | 0–1 天 | 1 天 |
| M3A create 与幂等 v2 | create 绑定、`CreateResult`、DTO、snapshot v1/v2、并发 winner 重放 | 4–6 天 | 0–1 天 | 1–2 天 |
| 5A-I 实现门禁 | 上传、未绑定删除、create 绑定、普通 PATCH 无损、鉴权下载、v2 幂等、基础启动恢复与 watchdog 的本机 smoke/crash | 3–5 天 | 1–2 天 | 1–2 天 |
| 5A-D 部署门禁（条件） | 三产物矩阵、cleanup 调度、进程监督、成套回退、ENOSPC、Windows/Linux 证据与发布说明 | 2–4 天 | 2–5 天 | 1–2 天 |
| M1B 组 journal 与恢复 | REMOVING、group journal、edit-remove/ORDER_DELETE、orphan claim 与完整组恢复 | 4–6 天 | 1–2 天 | 1–2 天 |
| M3B edit 与删除原语 | edit 组移除、窗口语义、并发不变量、订单删除 cleanup 原语 | 5–8 天 | 1–2 天 | 1–2 天 |
| M4 运维闭环 | 完整 cleanup/orphan、seed/reset、readiness 扩展和完整 crash smoke | 4–6 天 | 1–3 天 | 1–2 天 |
| 5B 最终集成 | 新目录 app smoke、最终部署 approval、回退与平台证据 | 2–4 天 | 2–4 天 | 1–2 天 |
| R 文档发布 | 事实源迁移、恢复手册、全仓/文档/基线 diff Evidence | 1–3 天 | 1–2 天 | 1 天 |

默认按 5A/5B 两批交付，不把拆分作为压缩工期时的可选项。当前各项数字均为条件 ROM，不是排期或阶段日期承诺；单人完整开发范围暂按 40–66 个开发日管理，环境等待和 review/修复缓冲单独计算，不能并入开发数字后隐藏。P0 的 6–10 天仅在无需自定义 syscall 且 Windows/Linux 原型一次通过时成立；若 Windows 需要专用打开/rename 层，P0 和 M1A 都必须单独增加缓冲。P0 八个工作包结束后，必须用实测结果替换上表原 ROM，而不是只追加说明；重估覆盖 M1A、M2、M3A、5A-D、M1B、M3B、M4、5B 和 R，并记录 owner、review 带宽、环境可用性、关键路径和资源不足时的降级顺序。重估完成前不得承诺交付日期。降级顺序固定为先取消持续部署批准，再推迟 5B edit/order-delete/orphan，最后才讨论缩小支持规模，恢复、安全、race 和 Evidence 门禁不得降级。并行时由单一 owner 持有 file/journal/recovery 协议，另一 owner 持有 HTTP/order integration；事务协议变更仍须串行 review。5A 只实现 STAGING、单项 delete/expiry、create bind 和对应恢复；5A binary 对 group/REMOVING/ORDER_DELETE/ORPHAN_FINAL 等不支持状态在构造 handler 前 fail-closed。5B 再交付 group journal、edit 组移除、完整 cleanup/orphan、订单删除 cleanup 原语和完整 crash matrix。
...
- [配套 checklist](./0005-2026-07-13-checklist.md) 的所有适用项完成并记录实际 Evidence；未持续部署 5A 时，5A-D 必须按批准的 N/A 流程关闭，并由最终 5B deployment approval 取代；
- `go test ./... -count=1`、`go vet ./...`、独立 `go test -race ./... -count=1` 通过；
- 冻结阶段五审计 baseline/merge base，记录 baseline 与 HEAD，并同时通过 `git diff --check <baseline>...HEAD` 和工作树 `git diff --check`；`docs/audit/validate.ps1`、`docs/audit/validate.tests.ps1` 同时通过；
- 新数据目录按入口矩阵完成 v6 bootstrap `migrate` → v7 schema-only `migrate` → seed → 5B API 启动 → 登录 → 上传 → 创建绑定 → 下载 → 编辑替换/移除 → 删除未绑定 → cleanup → 重启 smoke；
- 上传失败、DB 提交失败、rename/remove/open/hash 失败以及进程在每个 rename/commit 边界退出后，均无可访问孤儿、BOUND 缺失 final 或路径泄露；
- 5A-I、按实际部署路径适用的 5A-D 或其批准 N/A、最终 5B deployment approval、schema-only 限制、回退恢复、远端 Linux CI、最终 revision 和剩余风险有记录；
...
本计划对根目录两份 2026-07-14 临时审计意见逐条复核，结论为“需要修改，完成 P0 Evidence 后方可进入 M1A”。本轮采纳并落地：阶段五只交付供阶段六 UploadAction 消费的 HTTP 契约；fresh v0→v6→v7 与 legacy v6/v7 的真实 binary 入口矩阵；v6 gate 的精确对象/data 扫描；API/admin 同源码 revision、分别校验 SHA 且迁移前后固定 admin SHA；`StartupError`/`RunResult`/`ExitReason` 和逆序资源释放；Git ignored `artifacts/` 仅作缓存、tracked run index + 保留存储 + clean-checkout validator 样例；P0-1 至 P0-24 到八个 owner/reviewer 工作包的唯一映射；5A-D 条件 N/A 与最终 5B deployment approval；阶段 baseline diff；拆分 M4/5B/R 和环境/review 缓冲。既有独立 `order_attachments`、durable journal、v1/v2 snapshot、原始 multipart header、下载有界缓冲、stable applock 和 fail-closed 恢复主线继续保留。

未直接采信审计报告中声称已经通过的 test、race、validator 或 diff 基线；这些结果不作为本计划 Evidence，仍由 P0 在计划入库后的准确 revision 实跑。没有采纳“仅凭文件名或人工 manifest 区分产物”，也没有把 Windows 与 Linux 同时宣称为支持部署环境；只有完成监督、调度、恢复和 ENOSPC 证据的 profile 才能进入部署批准。对 5A 磁盘风险继续采用“前移最小 cleanup + 无部署证据时禁止持续部署”，不在本阶段引入完整配额系统。5A-I 通过后可继续 5B 开发；若实际持续部署 5A，则 5A-D 必须先通过，否则以批准的 N/A 关闭并由最终 5B deployment approval 替代。P0-1、fresh/legacy migration 执行链、journal 外键语义、startup/exit 协议、启动规模、Evidence validator/留存、完整锁矩阵和平台 harness 完成前保持 No-Go。
