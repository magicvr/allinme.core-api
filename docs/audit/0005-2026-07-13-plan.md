...
| `v7-5a-feature` | `phase5-v7-5a-api-<goos>-<goarch><ext>` 与 `phase5-v7-5a-admin-<goos>-<goarch><ext>`；分别执行 `go build -trimpath -o <api-file> ./cmd/api`、`go build -trimpath -o <admin-file> ./cmd/admin` | v7；允许 5A 状态，5B group/kind fail-closed | schema 非 v7、无法恢复或出现不支持 group/kind | 第 3 步启用；功能写入后的回退只允许恢复同一时点成套备份，不得换回 schema-only binary |

每行 Evidence 固定放在 `artifacts/phase5/v7/<artifact-kind>/<revision>/evidence/`，并记录部署命令、数据库路径、备份标识、允许数据扫描结果、退出码和回退演练。每个 binary 通过编译注入或等价只读接口输出机器可验证的 capability manifest；部署前 validator 必须核对 manifest、文件 SHA-256、schema/data scan，并以“错误 binary + 错误 schema/数据”负向 smoke 证明拒绝。文件名只作为辅助标识。历史过渡产物以各自 revision/hash 关联；最终 5A test/race/smoke 的“同一候选 revision”不追溯约束前两行。
...
- 发布说明明确 5A 只适用于单进程、受控 demo/内部环境，不构成生产或多副本批准，且 ENOSPC 收敛不等价于容量保护。
