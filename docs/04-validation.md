...
| 附件 | yes | `go test ./internal/files ./internal/order ./internal/store ./internal/httpapi ./internal/app ./internal/admin -count=1` | 临时目录覆盖 10 MiB 边界、类型伪造、危险文件名、摘要、本人与过期/绑定状态、鉴权下载、补偿删除、24 小时 cleanup、development seed/reset；真实 HTTP 覆盖上传 → 创建绑定 → 幂等重放 → list/detail 重查 → viewer 下载 → 删除未绑定附件 → 重启后下载 |
