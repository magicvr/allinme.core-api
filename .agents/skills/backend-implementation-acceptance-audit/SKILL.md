...
5. 不得采用 IMP 或既有审计的完成声明代替独立复核，不得自动归档计划；必须记录 `independence_basis`、干净且与 baseline 相同的 `evidence_revision`、由 IMP/REM 链派生的 `effective_result_revision`，以及本次运行唯一的 `evidence_run_id`。
