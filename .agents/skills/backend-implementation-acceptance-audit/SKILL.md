...
5. 必须由运行时创建不同于 implementer、实施审计、整改和 follow-up 的新 task/agent，并提供真实 `CONTEXT_ID`；缺失时停止，禁止本地生成 UUID 冒充隔离。记录上下文、线性派生的 `effective_result_revision` 和唯一 `evidence_run_id`。
