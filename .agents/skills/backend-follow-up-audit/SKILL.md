...
6. Resume a matching open follow-up for the same REM/result revision before allocating. Otherwise create a new follow-up AUD. On partial or failed remediation, move the queue to it; never append to source records.
7. Stop and report the missing canonical prompt if it cannot be read.
8. 生成的后续复审审计记录和最终报告必须使用中文；代码、命令、路径、ID 及固定的 frontmatter/status 值保持原样。
