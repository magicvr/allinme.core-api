...
2. 将 `TARGET` 解析为不可变集合并确定阶段；优先恢复 open AUD、in-progress IMP 和待处理 REM。存在关联 blocked/not-ready REM 时停止并报告恢复条件，不得重复整改。
3. 对尚无实施尝试的计划，逐个在独立上下文调用 `$backend-plan-acceptance-audit TARGET=<单一计划>`；非 `ready` 时调用 `$backend-plan-audit-until-ready TARGET=<单一计划> GOAL_MODE=child`。已有 IMP 的计划必须验证其记录的 ready 验收与当前 plan/checklist revision 是否仍有效；无漂移时不得为了刷新门禁重复创建验收，有漂移时必须先以 child 模式取得单计划 ready 验收，再判断原 IMP/REM 链是否仍覆盖新计划范围。
...
5. 仅对 `status=completed` 且 `audit=pending`、同时不存在匹配 open 审计的 IMP，逐个调用 `$backend-implementation-audit TARGET=<单一 IMP>`；禁止多个 IMP 共用一个实施审计 AUD。
6. 从索引派生该集合全部 `remediation=required` AUD，包括实施审计、follow-up 和失败的实施完成验收 AUD；调用 `$backend-fix-audit-findings TARGET=<AUD 列表>`。`remediation=decision-required` 立即停止。随后在新上下文逐个调用 `$backend-follow-up-audit TARGET=<单一 REM>`。实施 REM 必须形成线性祖先链。
