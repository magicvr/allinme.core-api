...
4. 实施审计、follow-up 和完成验收必须由运行时创建真实新 task/agent，再传入其真实 `CONTEXT_REF` 与 evidence correlation `CONTEXT_ID`；不得在当前上下文只换 UUID。每个 child 必须返回 clean terminal `governance_revision` 后才能继续。
5. 一个外层 cycle 对每个计划至多推进一个持久化状态；按 canonical state fingerprint 分计划计算 stagnant counter，默认 `MAX_CYCLES=12`，并隔离阻断和恢复入口。
