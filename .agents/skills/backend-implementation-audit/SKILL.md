...
description: Audit completed implementations with one resumable AUD per IMP, checking traceability, code, tests, evidence, CI, recovery, and release gates.
---
...
3. 将调用文本解释为可选 `TARGET`、`AUDITOR`、`CONTEXT_ID` 和 `FOCUS`；默认 `TARGET=pending`。`FOCUS` 只能增加深度。
...
6. 多 IMP 调用仅作为分派入口；每个 IMP 必须创建或恢复一份独立 AUD。预留前先恢复相同 IMP/baseline 的唯一 open 审计。
7. 每个失败 Control 必须关联当前 AUD finding，并保留独立验证证据。
