...
func TestCreateNormalizesCalculatesAndPersists(t *testing.T) {
	...
	_, err = service.Create(context.Background(), auth.Principal{Role: auth.RoleOperator}, order.CreateCommand{
...
func TestCreateAndEditValidationAndAuthorization(t *testing.T) {
	...
	if _, err := service.Create(context.Background(), auth.Principal{Role: auth.RoleViewer}, valid); !errors.Is(err, order.ErrForbidden) {
	...
	if _, err := service.Create(context.Background(), auth.Principal{Role: auth.RoleAdmin}, invalid); err == nil {
	...
	if _, err := service.Create(context.Background(), auth.Principal{Role: auth.RoleAdmin}, overflow); err == nil {
...
type writeRepository struct {
	created order.CreatePersistence
...
func (repository *writeRepository) CreateOrder(_ context.Context, persistence order.CreatePersistence) (order.Order, error) {
