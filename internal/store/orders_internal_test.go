...
func TestOrderRepositoryListUsesBoundedQueriesAndStableFiltering(t *testing.T) {
	...
	if queries != 5 || page.Total != 1 || len(page.Items) != 1 || page.Items[0].Items != nil {
	...
	if queries != 5 || page.Total != 306 || len(page.Items) != 100 {
...
func TestOrderRepositoryListCountAndPageShareSnapshot(t *testing.T) {
	...
	case listed := <-result:
		...
		if queries != 5 || listed.page.Total != 6 || len(listed.page.Items) != 6 {
