...
type uploadFixtureCase struct {
	ID       string                         `json:"id"`
	Input    protocol.UploadExecutionInput  `json:"input"`
	Expected protocol.UploadExecutionResult `json:"expected"`
}

type uploadFixtureSuite struct {
	FixtureVersion string              `json:"fixtureVersion"`
	Cases          []uploadFixtureCase `json:"cases"`
}

func TestUploadExecutionConformance(t *testing.T) {
	fixturesRoot := os.Getenv("SCHEMA_UI_FIXTURES")
	if fixturesRoot == "" {
		fixturesRoot = filepath.Join("..", "..", "..", "schema-ui-docs", "conformance", "fixtures")
	}
	contents, err := os.ReadFile(filepath.Join(fixturesRoot, "uploads", "cases.json"))
	if err != nil {
		t.Fatalf("read fixtures: %v", err)
	}

	var suite uploadFixtureSuite
	...
	for _, fixture := range suite.Cases {
		t.Run(fixture.ID, func(t *testing.T) {
			actual := protocol.ExecuteUpload(fixture.Input)
